...
O modelo roda no llama-swap local (porta 12434), usando o endpoint OpenAI-compatible.
...
#+RESULTS:
: 🤖 LangChain é um framework projetado para facilitar a criação de aplicações que utilizam Modelos de Linguagem (LLMs), permitindo "encadear" diferentes componentes para criar fluxos de trabalho complexos e conectá-los a dados externos.
